...
//+++Сделать калькулятор арабских цифр
////+++конвертация чисел из string в int

//+++Проверка на то, что римское число не больше 10

//+++Сделать проверку ввода в самом начале, которая проверяет римские это цифры или арабские( и вызывает соответсвующую функцию)
////Можно пропустить первую цифру числа по циклу сравнивая её с имеющимеся значениями в мапе, чтобы проверить римское или арабское это число

//+++Сделать проверку знаков (выводить ошибку на отсутствующие знаки)

//+++ДОБАВИТЬ ДОПОЛНИТЕЛЬНЫЕ ОБОЗНАЧЕНИЙ РИМСКИХ ЦИФР (10 - 100), для грамотного отбражения значения 10*10
//+++Выяснить почему программа понимает почему у меня написано число II, хотя в словаре ничего такого не указано - наисать это сюда
////Программа понимает почему у меня любое другое число, значение которого не прописанно в таблице, потому что я использую принцип, на котором основывается чтение римских чисел: меньше следующего числа - вычитанем и т.д

//!!!!Проблема кода в том, что он не проверяет знак римского числа при вводе, то есть оно может быть отрицательное, а код будет откидвать минус перед числом!!!!!
